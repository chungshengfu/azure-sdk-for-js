/*
 * Copyright (c) Microsoft Corporation.
 * Licensed under the MIT License.
 *
 * Code generated by Microsoft (R) AutoRest Code Generator.
 * Changes may cause incorrect behavior and will be lost if the code is regenerated.
 */

import { CassandraRepair } from "../operationsInterfaces";
import * as coreClient from "@azure/core-client";
import * as Mappers from "../models/mappers";
import * as Parameters from "../models/parameters";
import { CosmosDBManagementClient } from "../cosmosDBManagementClient";
import {
  SimplePollerLike,
  OperationState,
  createHttpPoller
} from "@azure/core-lro";
import { createLroSpec } from "../lroImpl";
import {
  CassandraRepairGetClusterStatusOptionalParams,
  CassandraRepairGetClusterStatusResponse,
  CassandraRepairGetTableStatusOptionalParams,
  CassandraRepairGetTableStatusResponse,
  CassandraClusterRepairPublicResource,
  CassandraRepairCreateOptionalParams,
  CassandraRepairCreateResponse,
  CassandraRepairDeleteOptionalParams,
  CassandraRepairDeleteResponse,
  CassandraRepairListOptionalParams,
  CassandraRepairListResponse,
  CassandraRepairPauseOptionalParams,
  CassandraRepairPauseResponse,
  CassandraRepairResumeOptionalParams,
  CassandraRepairResumeResponse,
  CassandraRepairListSegmentsOptionalParams,
  CassandraRepairListSegmentsResponse,
  CassandraRepairShowOptionalParams,
  CassandraRepairShowResponse,
  CassandraRepairAbortSegmentOptionalParams,
  CassandraRepairAbortSegmentResponse,
  CassandraRepairUpdateOptionalParams,
  CassandraRepairUpdateResponse
} from "../models";

/** Class containing CassandraRepair operations. */
export class CassandraRepairImpl implements CassandraRepair {
  private readonly client: CosmosDBManagementClient;

  /**
   * Initialize a new instance of the class CassandraRepair class.
   * @param client Reference to the service client
   */
  constructor(client: CosmosDBManagementClient) {
    this.client = client;
  }

  /**
   * @param resourceGroupName The name of the resource group. The name is case insensitive.
   * @param clusterName Managed Cassandra cluster name.
   * @param options The options parameters.
   */
  async beginGetClusterStatus(
    resourceGroupName: string,
    clusterName: string,
    options?: CassandraRepairGetClusterStatusOptionalParams
  ): Promise<
    SimplePollerLike<
      OperationState<CassandraRepairGetClusterStatusResponse>,
      CassandraRepairGetClusterStatusResponse
    >
  > {
    const directSendOperation = async (
      args: coreClient.OperationArguments,
      spec: coreClient.OperationSpec
    ): Promise<CassandraRepairGetClusterStatusResponse> => {
      return this.client.sendOperationRequest(args, spec);
    };
    const sendOperationFn = async (
      args: coreClient.OperationArguments,
      spec: coreClient.OperationSpec
    ) => {
      let currentRawResponse:
        | coreClient.FullOperationResponse
        | undefined = undefined;
      const providedCallback = args.options?.onResponse;
      const callback: coreClient.RawResponseCallback = (
        rawResponse: coreClient.FullOperationResponse,
        flatResponse: unknown
      ) => {
        currentRawResponse = rawResponse;
        providedCallback?.(rawResponse, flatResponse);
      };
      const updatedArgs = {
        ...args,
        options: {
          ...args.options,
          onResponse: callback
        }
      };
      const flatResponse = await directSendOperation(updatedArgs, spec);
      return {
        flatResponse,
        rawResponse: {
          statusCode: currentRawResponse!.status,
          body: currentRawResponse!.parsedBody,
          headers: currentRawResponse!.headers.toJSON()
        }
      };
    };

    const lro = createLroSpec({
      sendOperationFn,
      args: { resourceGroupName, clusterName, options },
      spec: getClusterStatusOperationSpec
    });
    const poller = await createHttpPoller<
      CassandraRepairGetClusterStatusResponse,
      OperationState<CassandraRepairGetClusterStatusResponse>
    >(lro, {
      restoreFrom: options?.resumeFrom,
      intervalInMs: options?.updateIntervalInMs,
      resourceLocationConfig: "location"
    });
    await poller.poll();
    return poller;
  }

  /**
   * @param resourceGroupName The name of the resource group. The name is case insensitive.
   * @param clusterName Managed Cassandra cluster name.
   * @param options The options parameters.
   */
  async beginGetClusterStatusAndWait(
    resourceGroupName: string,
    clusterName: string,
    options?: CassandraRepairGetClusterStatusOptionalParams
  ): Promise<CassandraRepairGetClusterStatusResponse> {
    const poller = await this.beginGetClusterStatus(
      resourceGroupName,
      clusterName,
      options
    );
    return poller.pollUntilDone();
  }

  /**
   * @param resourceGroupName The name of the resource group. The name is case insensitive.
   * @param clusterName Managed Cassandra cluster name.
   * @param options The options parameters.
   */
  async beginGetTableStatus(
    resourceGroupName: string,
    clusterName: string,
    options?: CassandraRepairGetTableStatusOptionalParams
  ): Promise<
    SimplePollerLike<
      OperationState<CassandraRepairGetTableStatusResponse>,
      CassandraRepairGetTableStatusResponse
    >
  > {
    const directSendOperation = async (
      args: coreClient.OperationArguments,
      spec: coreClient.OperationSpec
    ): Promise<CassandraRepairGetTableStatusResponse> => {
      return this.client.sendOperationRequest(args, spec);
    };
    const sendOperationFn = async (
      args: coreClient.OperationArguments,
      spec: coreClient.OperationSpec
    ) => {
      let currentRawResponse:
        | coreClient.FullOperationResponse
        | undefined = undefined;
      const providedCallback = args.options?.onResponse;
      const callback: coreClient.RawResponseCallback = (
        rawResponse: coreClient.FullOperationResponse,
        flatResponse: unknown
      ) => {
        currentRawResponse = rawResponse;
        providedCallback?.(rawResponse, flatResponse);
      };
      const updatedArgs = {
        ...args,
        options: {
          ...args.options,
          onResponse: callback
        }
      };
      const flatResponse = await directSendOperation(updatedArgs, spec);
      return {
        flatResponse,
        rawResponse: {
          statusCode: currentRawResponse!.status,
          body: currentRawResponse!.parsedBody,
          headers: currentRawResponse!.headers.toJSON()
        }
      };
    };

    const lro = createLroSpec({
      sendOperationFn,
      args: { resourceGroupName, clusterName, options },
      spec: getTableStatusOperationSpec
    });
    const poller = await createHttpPoller<
      CassandraRepairGetTableStatusResponse,
      OperationState<CassandraRepairGetTableStatusResponse>
    >(lro, {
      restoreFrom: options?.resumeFrom,
      intervalInMs: options?.updateIntervalInMs,
      resourceLocationConfig: "location"
    });
    await poller.poll();
    return poller;
  }

  /**
   * @param resourceGroupName The name of the resource group. The name is case insensitive.
   * @param clusterName Managed Cassandra cluster name.
   * @param options The options parameters.
   */
  async beginGetTableStatusAndWait(
    resourceGroupName: string,
    clusterName: string,
    options?: CassandraRepairGetTableStatusOptionalParams
  ): Promise<CassandraRepairGetTableStatusResponse> {
    const poller = await this.beginGetTableStatus(
      resourceGroupName,
      clusterName,
      options
    );
    return poller.pollUntilDone();
  }

  /**
   * @param resourceGroupName The name of the resource group. The name is case insensitive.
   * @param clusterName Managed Cassandra cluster name.
   * @param body The properties specifying the desired state of repairRun to be created.
   * @param options The options parameters.
   */
  async beginCreate(
    resourceGroupName: string,
    clusterName: string,
    body: CassandraClusterRepairPublicResource,
    options?: CassandraRepairCreateOptionalParams
  ): Promise<
    SimplePollerLike<
      OperationState<CassandraRepairCreateResponse>,
      CassandraRepairCreateResponse
    >
  > {
    const directSendOperation = async (
      args: coreClient.OperationArguments,
      spec: coreClient.OperationSpec
    ): Promise<CassandraRepairCreateResponse> => {
      return this.client.sendOperationRequest(args, spec);
    };
    const sendOperationFn = async (
      args: coreClient.OperationArguments,
      spec: coreClient.OperationSpec
    ) => {
      let currentRawResponse:
        | coreClient.FullOperationResponse
        | undefined = undefined;
      const providedCallback = args.options?.onResponse;
      const callback: coreClient.RawResponseCallback = (
        rawResponse: coreClient.FullOperationResponse,
        flatResponse: unknown
      ) => {
        currentRawResponse = rawResponse;
        providedCallback?.(rawResponse, flatResponse);
      };
      const updatedArgs = {
        ...args,
        options: {
          ...args.options,
          onResponse: callback
        }
      };
      const flatResponse = await directSendOperation(updatedArgs, spec);
      return {
        flatResponse,
        rawResponse: {
          statusCode: currentRawResponse!.status,
          body: currentRawResponse!.parsedBody,
          headers: currentRawResponse!.headers.toJSON()
        }
      };
    };

    const lro = createLroSpec({
      sendOperationFn,
      args: { resourceGroupName, clusterName, body, options },
      spec: createOperationSpec
    });
    const poller = await createHttpPoller<
      CassandraRepairCreateResponse,
      OperationState<CassandraRepairCreateResponse>
    >(lro, {
      restoreFrom: options?.resumeFrom,
      intervalInMs: options?.updateIntervalInMs,
      resourceLocationConfig: "location"
    });
    await poller.poll();
    return poller;
  }

  /**
   * @param resourceGroupName The name of the resource group. The name is case insensitive.
   * @param clusterName Managed Cassandra cluster name.
   * @param body The properties specifying the desired state of repairRun to be created.
   * @param options The options parameters.
   */
  async beginCreateAndWait(
    resourceGroupName: string,
    clusterName: string,
    body: CassandraClusterRepairPublicResource,
    options?: CassandraRepairCreateOptionalParams
  ): Promise<CassandraRepairCreateResponse> {
    const poller = await this.beginCreate(
      resourceGroupName,
      clusterName,
      body,
      options
    );
    return poller.pollUntilDone();
  }

  /**
   * @param resourceGroupName The name of the resource group. The name is case insensitive.
   * @param clusterName Managed Cassandra cluster name.
   * @param repairRunId Id of repair run created to repair Cassandra cluster.
   * @param ownerName Owner Name that was used to create the Repair Run.
   * @param options The options parameters.
   */
  async beginDelete(
    resourceGroupName: string,
    clusterName: string,
    repairRunId: string,
    ownerName: string,
    options?: CassandraRepairDeleteOptionalParams
  ): Promise<
    SimplePollerLike<
      OperationState<CassandraRepairDeleteResponse>,
      CassandraRepairDeleteResponse
    >
  > {
    const directSendOperation = async (
      args: coreClient.OperationArguments,
      spec: coreClient.OperationSpec
    ): Promise<CassandraRepairDeleteResponse> => {
      return this.client.sendOperationRequest(args, spec);
    };
    const sendOperationFn = async (
      args: coreClient.OperationArguments,
      spec: coreClient.OperationSpec
    ) => {
      let currentRawResponse:
        | coreClient.FullOperationResponse
        | undefined = undefined;
      const providedCallback = args.options?.onResponse;
      const callback: coreClient.RawResponseCallback = (
        rawResponse: coreClient.FullOperationResponse,
        flatResponse: unknown
      ) => {
        currentRawResponse = rawResponse;
        providedCallback?.(rawResponse, flatResponse);
      };
      const updatedArgs = {
        ...args,
        options: {
          ...args.options,
          onResponse: callback
        }
      };
      const flatResponse = await directSendOperation(updatedArgs, spec);
      return {
        flatResponse,
        rawResponse: {
          statusCode: currentRawResponse!.status,
          body: currentRawResponse!.parsedBody,
          headers: currentRawResponse!.headers.toJSON()
        }
      };
    };

    const lro = createLroSpec({
      sendOperationFn,
      args: { resourceGroupName, clusterName, repairRunId, ownerName, options },
      spec: deleteOperationSpec
    });
    const poller = await createHttpPoller<
      CassandraRepairDeleteResponse,
      OperationState<CassandraRepairDeleteResponse>
    >(lro, {
      restoreFrom: options?.resumeFrom,
      intervalInMs: options?.updateIntervalInMs
    });
    await poller.poll();
    return poller;
  }

  /**
   * @param resourceGroupName The name of the resource group. The name is case insensitive.
   * @param clusterName Managed Cassandra cluster name.
   * @param repairRunId Id of repair run created to repair Cassandra cluster.
   * @param ownerName Owner Name that was used to create the Repair Run.
   * @param options The options parameters.
   */
  async beginDeleteAndWait(
    resourceGroupName: string,
    clusterName: string,
    repairRunId: string,
    ownerName: string,
    options?: CassandraRepairDeleteOptionalParams
  ): Promise<CassandraRepairDeleteResponse> {
    const poller = await this.beginDelete(
      resourceGroupName,
      clusterName,
      repairRunId,
      ownerName,
      options
    );
    return poller.pollUntilDone();
  }

  /**
   * @param resourceGroupName The name of the resource group. The name is case insensitive.
   * @param clusterName Managed Cassandra cluster name.
   * @param options The options parameters.
   */
  async beginList(
    resourceGroupName: string,
    clusterName: string,
    options?: CassandraRepairListOptionalParams
  ): Promise<
    SimplePollerLike<
      OperationState<CassandraRepairListResponse>,
      CassandraRepairListResponse
    >
  > {
    const directSendOperation = async (
      args: coreClient.OperationArguments,
      spec: coreClient.OperationSpec
    ): Promise<CassandraRepairListResponse> => {
      return this.client.sendOperationRequest(args, spec);
    };
    const sendOperationFn = async (
      args: coreClient.OperationArguments,
      spec: coreClient.OperationSpec
    ) => {
      let currentRawResponse:
        | coreClient.FullOperationResponse
        | undefined = undefined;
      const providedCallback = args.options?.onResponse;
      const callback: coreClient.RawResponseCallback = (
        rawResponse: coreClient.FullOperationResponse,
        flatResponse: unknown
      ) => {
        currentRawResponse = rawResponse;
        providedCallback?.(rawResponse, flatResponse);
      };
      const updatedArgs = {
        ...args,
        options: {
          ...args.options,
          onResponse: callback
        }
      };
      const flatResponse = await directSendOperation(updatedArgs, spec);
      return {
        flatResponse,
        rawResponse: {
          statusCode: currentRawResponse!.status,
          body: currentRawResponse!.parsedBody,
          headers: currentRawResponse!.headers.toJSON()
        }
      };
    };

    const lro = createLroSpec({
      sendOperationFn,
      args: { resourceGroupName, clusterName, options },
      spec: listOperationSpec
    });
    const poller = await createHttpPoller<
      CassandraRepairListResponse,
      OperationState<CassandraRepairListResponse>
    >(lro, {
      restoreFrom: options?.resumeFrom,
      intervalInMs: options?.updateIntervalInMs,
      resourceLocationConfig: "location"
    });
    await poller.poll();
    return poller;
  }

  /**
   * @param resourceGroupName The name of the resource group. The name is case insensitive.
   * @param clusterName Managed Cassandra cluster name.
   * @param options The options parameters.
   */
  async beginListAndWait(
    resourceGroupName: string,
    clusterName: string,
    options?: CassandraRepairListOptionalParams
  ): Promise<CassandraRepairListResponse> {
    const poller = await this.beginList(
      resourceGroupName,
      clusterName,
      options
    );
    return poller.pollUntilDone();
  }

  /**
   * @param resourceGroupName The name of the resource group. The name is case insensitive.
   * @param clusterName Managed Cassandra cluster name.
   * @param repairRunId Id of repair run created to repair Cassandra cluster.
   * @param options The options parameters.
   */
  async beginPause(
    resourceGroupName: string,
    clusterName: string,
    repairRunId: string,
    options?: CassandraRepairPauseOptionalParams
  ): Promise<
    SimplePollerLike<
      OperationState<CassandraRepairPauseResponse>,
      CassandraRepairPauseResponse
    >
  > {
    const directSendOperation = async (
      args: coreClient.OperationArguments,
      spec: coreClient.OperationSpec
    ): Promise<CassandraRepairPauseResponse> => {
      return this.client.sendOperationRequest(args, spec);
    };
    const sendOperationFn = async (
      args: coreClient.OperationArguments,
      spec: coreClient.OperationSpec
    ) => {
      let currentRawResponse:
        | coreClient.FullOperationResponse
        | undefined = undefined;
      const providedCallback = args.options?.onResponse;
      const callback: coreClient.RawResponseCallback = (
        rawResponse: coreClient.FullOperationResponse,
        flatResponse: unknown
      ) => {
        currentRawResponse = rawResponse;
        providedCallback?.(rawResponse, flatResponse);
      };
      const updatedArgs = {
        ...args,
        options: {
          ...args.options,
          onResponse: callback
        }
      };
      const flatResponse = await directSendOperation(updatedArgs, spec);
      return {
        flatResponse,
        rawResponse: {
          statusCode: currentRawResponse!.status,
          body: currentRawResponse!.parsedBody,
          headers: currentRawResponse!.headers.toJSON()
        }
      };
    };

    const lro = createLroSpec({
      sendOperationFn,
      args: { resourceGroupName, clusterName, repairRunId, options },
      spec: pauseOperationSpec
    });
    const poller = await createHttpPoller<
      CassandraRepairPauseResponse,
      OperationState<CassandraRepairPauseResponse>
    >(lro, {
      restoreFrom: options?.resumeFrom,
      intervalInMs: options?.updateIntervalInMs,
      resourceLocationConfig: "location"
    });
    await poller.poll();
    return poller;
  }

  /**
   * @param resourceGroupName The name of the resource group. The name is case insensitive.
   * @param clusterName Managed Cassandra cluster name.
   * @param repairRunId Id of repair run created to repair Cassandra cluster.
   * @param options The options parameters.
   */
  async beginPauseAndWait(
    resourceGroupName: string,
    clusterName: string,
    repairRunId: string,
    options?: CassandraRepairPauseOptionalParams
  ): Promise<CassandraRepairPauseResponse> {
    const poller = await this.beginPause(
      resourceGroupName,
      clusterName,
      repairRunId,
      options
    );
    return poller.pollUntilDone();
  }

  /**
   * @param resourceGroupName The name of the resource group. The name is case insensitive.
   * @param clusterName Managed Cassandra cluster name.
   * @param repairRunId Id of repair run created to repair Cassandra cluster.
   * @param options The options parameters.
   */
  async beginResume(
    resourceGroupName: string,
    clusterName: string,
    repairRunId: string,
    options?: CassandraRepairResumeOptionalParams
  ): Promise<
    SimplePollerLike<
      OperationState<CassandraRepairResumeResponse>,
      CassandraRepairResumeResponse
    >
  > {
    const directSendOperation = async (
      args: coreClient.OperationArguments,
      spec: coreClient.OperationSpec
    ): Promise<CassandraRepairResumeResponse> => {
      return this.client.sendOperationRequest(args, spec);
    };
    const sendOperationFn = async (
      args: coreClient.OperationArguments,
      spec: coreClient.OperationSpec
    ) => {
      let currentRawResponse:
        | coreClient.FullOperationResponse
        | undefined = undefined;
      const providedCallback = args.options?.onResponse;
      const callback: coreClient.RawResponseCallback = (
        rawResponse: coreClient.FullOperationResponse,
        flatResponse: unknown
      ) => {
        currentRawResponse = rawResponse;
        providedCallback?.(rawResponse, flatResponse);
      };
      const updatedArgs = {
        ...args,
        options: {
          ...args.options,
          onResponse: callback
        }
      };
      const flatResponse = await directSendOperation(updatedArgs, spec);
      return {
        flatResponse,
        rawResponse: {
          statusCode: currentRawResponse!.status,
          body: currentRawResponse!.parsedBody,
          headers: currentRawResponse!.headers.toJSON()
        }
      };
    };

    const lro = createLroSpec({
      sendOperationFn,
      args: { resourceGroupName, clusterName, repairRunId, options },
      spec: resumeOperationSpec
    });
    const poller = await createHttpPoller<
      CassandraRepairResumeResponse,
      OperationState<CassandraRepairResumeResponse>
    >(lro, {
      restoreFrom: options?.resumeFrom,
      intervalInMs: options?.updateIntervalInMs,
      resourceLocationConfig: "location"
    });
    await poller.poll();
    return poller;
  }

  /**
   * @param resourceGroupName The name of the resource group. The name is case insensitive.
   * @param clusterName Managed Cassandra cluster name.
   * @param repairRunId Id of repair run created to repair Cassandra cluster.
   * @param options The options parameters.
   */
  async beginResumeAndWait(
    resourceGroupName: string,
    clusterName: string,
    repairRunId: string,
    options?: CassandraRepairResumeOptionalParams
  ): Promise<CassandraRepairResumeResponse> {
    const poller = await this.beginResume(
      resourceGroupName,
      clusterName,
      repairRunId,
      options
    );
    return poller.pollUntilDone();
  }

  /**
   * @param resourceGroupName The name of the resource group. The name is case insensitive.
   * @param clusterName Managed Cassandra cluster name.
   * @param repairRunId Id of repair run created to repair Cassandra cluster.
   * @param options The options parameters.
   */
  async beginListSegments(
    resourceGroupName: string,
    clusterName: string,
    repairRunId: string,
    options?: CassandraRepairListSegmentsOptionalParams
  ): Promise<
    SimplePollerLike<
      OperationState<CassandraRepairListSegmentsResponse>,
      CassandraRepairListSegmentsResponse
    >
  > {
    const directSendOperation = async (
      args: coreClient.OperationArguments,
      spec: coreClient.OperationSpec
    ): Promise<CassandraRepairListSegmentsResponse> => {
      return this.client.sendOperationRequest(args, spec);
    };
    const sendOperationFn = async (
      args: coreClient.OperationArguments,
      spec: coreClient.OperationSpec
    ) => {
      let currentRawResponse:
        | coreClient.FullOperationResponse
        | undefined = undefined;
      const providedCallback = args.options?.onResponse;
      const callback: coreClient.RawResponseCallback = (
        rawResponse: coreClient.FullOperationResponse,
        flatResponse: unknown
      ) => {
        currentRawResponse = rawResponse;
        providedCallback?.(rawResponse, flatResponse);
      };
      const updatedArgs = {
        ...args,
        options: {
          ...args.options,
          onResponse: callback
        }
      };
      const flatResponse = await directSendOperation(updatedArgs, spec);
      return {
        flatResponse,
        rawResponse: {
          statusCode: currentRawResponse!.status,
          body: currentRawResponse!.parsedBody,
          headers: currentRawResponse!.headers.toJSON()
        }
      };
    };

    const lro = createLroSpec({
      sendOperationFn,
      args: { resourceGroupName, clusterName, repairRunId, options },
      spec: listSegmentsOperationSpec
    });
    const poller = await createHttpPoller<
      CassandraRepairListSegmentsResponse,
      OperationState<CassandraRepairListSegmentsResponse>
    >(lro, {
      restoreFrom: options?.resumeFrom,
      intervalInMs: options?.updateIntervalInMs,
      resourceLocationConfig: "location"
    });
    await poller.poll();
    return poller;
  }

  /**
   * @param resourceGroupName The name of the resource group. The name is case insensitive.
   * @param clusterName Managed Cassandra cluster name.
   * @param repairRunId Id of repair run created to repair Cassandra cluster.
   * @param options The options parameters.
   */
  async beginListSegmentsAndWait(
    resourceGroupName: string,
    clusterName: string,
    repairRunId: string,
    options?: CassandraRepairListSegmentsOptionalParams
  ): Promise<CassandraRepairListSegmentsResponse> {
    const poller = await this.beginListSegments(
      resourceGroupName,
      clusterName,
      repairRunId,
      options
    );
    return poller.pollUntilDone();
  }

  /**
   * @param resourceGroupName The name of the resource group. The name is case insensitive.
   * @param clusterName Managed Cassandra cluster name.
   * @param repairRunId Id of repair run created to repair Cassandra cluster.
   * @param options The options parameters.
   */
  async beginShow(
    resourceGroupName: string,
    clusterName: string,
    repairRunId: string,
    options?: CassandraRepairShowOptionalParams
  ): Promise<
    SimplePollerLike<
      OperationState<CassandraRepairShowResponse>,
      CassandraRepairShowResponse
    >
  > {
    const directSendOperation = async (
      args: coreClient.OperationArguments,
      spec: coreClient.OperationSpec
    ): Promise<CassandraRepairShowResponse> => {
      return this.client.sendOperationRequest(args, spec);
    };
    const sendOperationFn = async (
      args: coreClient.OperationArguments,
      spec: coreClient.OperationSpec
    ) => {
      let currentRawResponse:
        | coreClient.FullOperationResponse
        | undefined = undefined;
      const providedCallback = args.options?.onResponse;
      const callback: coreClient.RawResponseCallback = (
        rawResponse: coreClient.FullOperationResponse,
        flatResponse: unknown
      ) => {
        currentRawResponse = rawResponse;
        providedCallback?.(rawResponse, flatResponse);
      };
      const updatedArgs = {
        ...args,
        options: {
          ...args.options,
          onResponse: callback
        }
      };
      const flatResponse = await directSendOperation(updatedArgs, spec);
      return {
        flatResponse,
        rawResponse: {
          statusCode: currentRawResponse!.status,
          body: currentRawResponse!.parsedBody,
          headers: currentRawResponse!.headers.toJSON()
        }
      };
    };

    const lro = createLroSpec({
      sendOperationFn,
      args: { resourceGroupName, clusterName, repairRunId, options },
      spec: showOperationSpec
    });
    const poller = await createHttpPoller<
      CassandraRepairShowResponse,
      OperationState<CassandraRepairShowResponse>
    >(lro, {
      restoreFrom: options?.resumeFrom,
      intervalInMs: options?.updateIntervalInMs,
      resourceLocationConfig: "location"
    });
    await poller.poll();
    return poller;
  }

  /**
   * @param resourceGroupName The name of the resource group. The name is case insensitive.
   * @param clusterName Managed Cassandra cluster name.
   * @param repairRunId Id of repair run created to repair Cassandra cluster.
   * @param options The options parameters.
   */
  async beginShowAndWait(
    resourceGroupName: string,
    clusterName: string,
    repairRunId: string,
    options?: CassandraRepairShowOptionalParams
  ): Promise<CassandraRepairShowResponse> {
    const poller = await this.beginShow(
      resourceGroupName,
      clusterName,
      repairRunId,
      options
    );
    return poller.pollUntilDone();
  }

  /**
   * @param resourceGroupName The name of the resource group. The name is case insensitive.
   * @param clusterName Managed Cassandra cluster name.
   * @param repairRunId Id of repair run created to repair Cassandra cluster.
   * @param segmentId Id of a segment of a repair run.
   * @param options The options parameters.
   */
  async beginAbortSegment(
    resourceGroupName: string,
    clusterName: string,
    repairRunId: string,
    segmentId: string,
    options?: CassandraRepairAbortSegmentOptionalParams
  ): Promise<
    SimplePollerLike<
      OperationState<CassandraRepairAbortSegmentResponse>,
      CassandraRepairAbortSegmentResponse
    >
  > {
    const directSendOperation = async (
      args: coreClient.OperationArguments,
      spec: coreClient.OperationSpec
    ): Promise<CassandraRepairAbortSegmentResponse> => {
      return this.client.sendOperationRequest(args, spec);
    };
    const sendOperationFn = async (
      args: coreClient.OperationArguments,
      spec: coreClient.OperationSpec
    ) => {
      let currentRawResponse:
        | coreClient.FullOperationResponse
        | undefined = undefined;
      const providedCallback = args.options?.onResponse;
      const callback: coreClient.RawResponseCallback = (
        rawResponse: coreClient.FullOperationResponse,
        flatResponse: unknown
      ) => {
        currentRawResponse = rawResponse;
        providedCallback?.(rawResponse, flatResponse);
      };
      const updatedArgs = {
        ...args,
        options: {
          ...args.options,
          onResponse: callback
        }
      };
      const flatResponse = await directSendOperation(updatedArgs, spec);
      return {
        flatResponse,
        rawResponse: {
          statusCode: currentRawResponse!.status,
          body: currentRawResponse!.parsedBody,
          headers: currentRawResponse!.headers.toJSON()
        }
      };
    };

    const lro = createLroSpec({
      sendOperationFn,
      args: { resourceGroupName, clusterName, repairRunId, segmentId, options },
      spec: abortSegmentOperationSpec
    });
    const poller = await createHttpPoller<
      CassandraRepairAbortSegmentResponse,
      OperationState<CassandraRepairAbortSegmentResponse>
    >(lro, {
      restoreFrom: options?.resumeFrom,
      intervalInMs: options?.updateIntervalInMs,
      resourceLocationConfig: "location"
    });
    await poller.poll();
    return poller;
  }

  /**
   * @param resourceGroupName The name of the resource group. The name is case insensitive.
   * @param clusterName Managed Cassandra cluster name.
   * @param repairRunId Id of repair run created to repair Cassandra cluster.
   * @param segmentId Id of a segment of a repair run.
   * @param options The options parameters.
   */
  async beginAbortSegmentAndWait(
    resourceGroupName: string,
    clusterName: string,
    repairRunId: string,
    segmentId: string,
    options?: CassandraRepairAbortSegmentOptionalParams
  ): Promise<CassandraRepairAbortSegmentResponse> {
    const poller = await this.beginAbortSegment(
      resourceGroupName,
      clusterName,
      repairRunId,
      segmentId,
      options
    );
    return poller.pollUntilDone();
  }

  /**
   * @param resourceGroupName The name of the resource group. The name is case insensitive.
   * @param clusterName Managed Cassandra cluster name.
   * @param repairRunId Id of repair run created to repair Cassandra cluster.
   * @param intensityValue Floating point value representing intensity of the repair.
   * @param options The options parameters.
   */
  async beginUpdate(
    resourceGroupName: string,
    clusterName: string,
    repairRunId: string,
    intensityValue: string,
    options?: CassandraRepairUpdateOptionalParams
  ): Promise<
    SimplePollerLike<
      OperationState<CassandraRepairUpdateResponse>,
      CassandraRepairUpdateResponse
    >
  > {
    const directSendOperation = async (
      args: coreClient.OperationArguments,
      spec: coreClient.OperationSpec
    ): Promise<CassandraRepairUpdateResponse> => {
      return this.client.sendOperationRequest(args, spec);
    };
    const sendOperationFn = async (
      args: coreClient.OperationArguments,
      spec: coreClient.OperationSpec
    ) => {
      let currentRawResponse:
        | coreClient.FullOperationResponse
        | undefined = undefined;
      const providedCallback = args.options?.onResponse;
      const callback: coreClient.RawResponseCallback = (
        rawResponse: coreClient.FullOperationResponse,
        flatResponse: unknown
      ) => {
        currentRawResponse = rawResponse;
        providedCallback?.(rawResponse, flatResponse);
      };
      const updatedArgs = {
        ...args,
        options: {
          ...args.options,
          onResponse: callback
        }
      };
      const flatResponse = await directSendOperation(updatedArgs, spec);
      return {
        flatResponse,
        rawResponse: {
          statusCode: currentRawResponse!.status,
          body: currentRawResponse!.parsedBody,
          headers: currentRawResponse!.headers.toJSON()
        }
      };
    };

    const lro = createLroSpec({
      sendOperationFn,
      args: {
        resourceGroupName,
        clusterName,
        repairRunId,
        intensityValue,
        options
      },
      spec: updateOperationSpec
    });
    const poller = await createHttpPoller<
      CassandraRepairUpdateResponse,
      OperationState<CassandraRepairUpdateResponse>
    >(lro, {
      restoreFrom: options?.resumeFrom,
      intervalInMs: options?.updateIntervalInMs,
      resourceLocationConfig: "location"
    });
    await poller.poll();
    return poller;
  }

  /**
   * @param resourceGroupName The name of the resource group. The name is case insensitive.
   * @param clusterName Managed Cassandra cluster name.
   * @param repairRunId Id of repair run created to repair Cassandra cluster.
   * @param intensityValue Floating point value representing intensity of the repair.
   * @param options The options parameters.
   */
  async beginUpdateAndWait(
    resourceGroupName: string,
    clusterName: string,
    repairRunId: string,
    intensityValue: string,
    options?: CassandraRepairUpdateOptionalParams
  ): Promise<CassandraRepairUpdateResponse> {
    const poller = await this.beginUpdate(
      resourceGroupName,
      clusterName,
      repairRunId,
      intensityValue,
      options
    );
    return poller.pollUntilDone();
  }
}
// Operation Specifications
const serializer = coreClient.createSerializer(Mappers, /* isXml */ false);

const getClusterStatusOperationSpec: coreClient.OperationSpec = {
  path:
    "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.DocumentDB/cassandraClusters/{clusterName}/repairStatus",
  httpMethod: "POST",
  responses: {
    200: {
      bodyMapper: Mappers.CassandraReaperClusterStatus
    },
    201: {
      bodyMapper: Mappers.CassandraReaperClusterStatus
    },
    202: {
      bodyMapper: Mappers.CassandraReaperClusterStatus
    },
    204: {
      bodyMapper: Mappers.CassandraReaperClusterStatus
    },
    default: {
      bodyMapper: Mappers.CloudError
    }
  },
  queryParameters: [Parameters.apiVersion],
  urlParameters: [
    Parameters.$host,
    Parameters.subscriptionId,
    Parameters.resourceGroupName,
    Parameters.clusterName
  ],
  headerParameters: [Parameters.accept],
  serializer
};
const getTableStatusOperationSpec: coreClient.OperationSpec = {
  path:
    "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.DocumentDB/cassandraClusters/{clusterName}/repairTables",
  httpMethod: "POST",
  responses: {
    200: {
      bodyMapper: { type: { name: "String" } }
    },
    201: {
      bodyMapper: { type: { name: "String" } }
    },
    202: {
      bodyMapper: { type: { name: "String" } }
    },
    204: {
      bodyMapper: { type: { name: "String" } }
    }
  },
  queryParameters: [Parameters.apiVersion],
  urlParameters: [
    Parameters.$host,
    Parameters.subscriptionId,
    Parameters.resourceGroupName,
    Parameters.clusterName
  ],
  headerParameters: [Parameters.accept],
  serializer
};
const createOperationSpec: coreClient.OperationSpec = {
  path:
    "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.DocumentDB/cassandraClusters/{clusterName}/repairRuns",
  httpMethod: "POST",
  responses: {
    200: {
      bodyMapper: Mappers.CassandraReaperRunStatus
    },
    201: {
      bodyMapper: Mappers.CassandraReaperRunStatus
    },
    202: {
      bodyMapper: Mappers.CassandraReaperRunStatus
    },
    204: {
      bodyMapper: Mappers.CassandraReaperRunStatus
    }
  },
  requestBody: Parameters.body3,
  queryParameters: [Parameters.apiVersion],
  urlParameters: [
    Parameters.$host,
    Parameters.subscriptionId,
    Parameters.resourceGroupName,
    Parameters.clusterName
  ],
  headerParameters: [Parameters.accept, Parameters.contentType],
  mediaType: "json",
  serializer
};
const deleteOperationSpec: coreClient.OperationSpec = {
  path:
    "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.DocumentDB/cassandraClusters/{clusterName}/repairRuns/{repairRunId}/owner/{ownerName}",
  httpMethod: "DELETE",
  responses: {
    200: {
      headersMapper: Mappers.CassandraRepairDeleteHeaders
    },
    201: {
      headersMapper: Mappers.CassandraRepairDeleteHeaders
    },
    202: {
      headersMapper: Mappers.CassandraRepairDeleteHeaders
    },
    204: {
      headersMapper: Mappers.CassandraRepairDeleteHeaders
    },
    default: {
      bodyMapper: Mappers.CloudError
    }
  },
  queryParameters: [Parameters.apiVersion],
  urlParameters: [
    Parameters.$host,
    Parameters.subscriptionId,
    Parameters.resourceGroupName,
    Parameters.clusterName,
    Parameters.repairRunId,
    Parameters.ownerName
  ],
  headerParameters: [Parameters.accept],
  serializer
};
const listOperationSpec: coreClient.OperationSpec = {
  path:
    "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.DocumentDB/cassandraClusters/{clusterName}/repairRuns/list",
  httpMethod: "POST",
  responses: {
    200: {
      bodyMapper: Mappers.CassandraReaperRunStatusFeedResponse
    },
    201: {
      bodyMapper: Mappers.CassandraReaperRunStatusFeedResponse
    },
    202: {
      bodyMapper: Mappers.CassandraReaperRunStatusFeedResponse
    },
    204: {
      bodyMapper: Mappers.CassandraReaperRunStatusFeedResponse
    },
    default: {
      bodyMapper: Mappers.CloudError
    }
  },
  requestBody: Parameters.body4,
  queryParameters: [Parameters.apiVersion],
  urlParameters: [
    Parameters.$host,
    Parameters.subscriptionId,
    Parameters.resourceGroupName,
    Parameters.clusterName
  ],
  headerParameters: [Parameters.accept, Parameters.contentType],
  mediaType: "json",
  serializer
};
const pauseOperationSpec: coreClient.OperationSpec = {
  path:
    "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.DocumentDB/cassandraClusters/{clusterName}/repairRuns/{repairRunId}/pause",
  httpMethod: "POST",
  responses: {
    200: {
      headersMapper: Mappers.CassandraRepairPauseHeaders
    },
    201: {
      headersMapper: Mappers.CassandraRepairPauseHeaders
    },
    202: {
      headersMapper: Mappers.CassandraRepairPauseHeaders
    },
    204: {
      headersMapper: Mappers.CassandraRepairPauseHeaders
    },
    default: {
      bodyMapper: Mappers.CloudError
    }
  },
  queryParameters: [Parameters.apiVersion],
  urlParameters: [
    Parameters.$host,
    Parameters.subscriptionId,
    Parameters.resourceGroupName,
    Parameters.clusterName,
    Parameters.repairRunId
  ],
  headerParameters: [Parameters.accept],
  serializer
};
const resumeOperationSpec: coreClient.OperationSpec = {
  path:
    "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.DocumentDB/cassandraClusters/{clusterName}/repairRuns/{repairRunId}/resume",
  httpMethod: "POST",
  responses: {
    200: {
      headersMapper: Mappers.CassandraRepairResumeHeaders
    },
    201: {
      headersMapper: Mappers.CassandraRepairResumeHeaders
    },
    202: {
      headersMapper: Mappers.CassandraRepairResumeHeaders
    },
    204: {
      headersMapper: Mappers.CassandraRepairResumeHeaders
    },
    default: {
      bodyMapper: Mappers.CloudError
    }
  },
  queryParameters: [Parameters.apiVersion],
  urlParameters: [
    Parameters.$host,
    Parameters.subscriptionId,
    Parameters.resourceGroupName,
    Parameters.clusterName,
    Parameters.repairRunId
  ],
  headerParameters: [Parameters.accept],
  serializer
};
const listSegmentsOperationSpec: coreClient.OperationSpec = {
  path:
    "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.DocumentDB/cassandraClusters/{clusterName}/repairRuns/{repairRunId}/segments",
  httpMethod: "POST",
  responses: {
    200: {
      bodyMapper: Mappers.CassandraRepairSegmentResourceFeedResponse
    },
    201: {
      bodyMapper: Mappers.CassandraRepairSegmentResourceFeedResponse
    },
    202: {
      bodyMapper: Mappers.CassandraRepairSegmentResourceFeedResponse
    },
    204: {
      bodyMapper: Mappers.CassandraRepairSegmentResourceFeedResponse
    },
    default: {
      bodyMapper: Mappers.CloudError
    }
  },
  queryParameters: [Parameters.apiVersion],
  urlParameters: [
    Parameters.$host,
    Parameters.subscriptionId,
    Parameters.resourceGroupName,
    Parameters.clusterName,
    Parameters.repairRunId
  ],
  headerParameters: [Parameters.accept],
  serializer
};
const showOperationSpec: coreClient.OperationSpec = {
  path:
    "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.DocumentDB/cassandraClusters/{clusterName}/repairRuns/{repairRunId}/show",
  httpMethod: "POST",
  responses: {
    200: {
      bodyMapper: Mappers.CassandraReaperRunStatus
    },
    201: {
      bodyMapper: Mappers.CassandraReaperRunStatus
    },
    202: {
      bodyMapper: Mappers.CassandraReaperRunStatus
    },
    204: {
      bodyMapper: Mappers.CassandraReaperRunStatus
    },
    default: {
      bodyMapper: Mappers.CloudError
    }
  },
  queryParameters: [Parameters.apiVersion],
  urlParameters: [
    Parameters.$host,
    Parameters.subscriptionId,
    Parameters.resourceGroupName,
    Parameters.clusterName,
    Parameters.repairRunId
  ],
  headerParameters: [Parameters.accept],
  serializer
};
const abortSegmentOperationSpec: coreClient.OperationSpec = {
  path:
    "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.DocumentDB/cassandraClusters/{clusterName}/repairRuns/{repairRunId}/segments/{segmentId}/abort",
  httpMethod: "POST",
  responses: {
    200: {
      headersMapper: Mappers.CassandraRepairAbortSegmentHeaders
    },
    201: {
      headersMapper: Mappers.CassandraRepairAbortSegmentHeaders
    },
    202: {
      headersMapper: Mappers.CassandraRepairAbortSegmentHeaders
    },
    204: {
      headersMapper: Mappers.CassandraRepairAbortSegmentHeaders
    },
    default: {
      bodyMapper: Mappers.CloudError
    }
  },
  queryParameters: [Parameters.apiVersion],
  urlParameters: [
    Parameters.$host,
    Parameters.subscriptionId,
    Parameters.resourceGroupName,
    Parameters.clusterName,
    Parameters.repairRunId,
    Parameters.segmentId
  ],
  headerParameters: [Parameters.accept],
  serializer
};
const updateOperationSpec: coreClient.OperationSpec = {
  path:
    "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.DocumentDB/cassandraClusters/{clusterName}/repairRuns/{repairRunId}/intensity/{intensityValue}",
  httpMethod: "POST",
  responses: {
    200: {
      headersMapper: Mappers.CassandraRepairUpdateHeaders
    },
    201: {
      headersMapper: Mappers.CassandraRepairUpdateHeaders
    },
    202: {
      headersMapper: Mappers.CassandraRepairUpdateHeaders
    },
    204: {
      headersMapper: Mappers.CassandraRepairUpdateHeaders
    },
    default: {
      bodyMapper: Mappers.CloudError
    }
  },
  queryParameters: [Parameters.apiVersion],
  urlParameters: [
    Parameters.$host,
    Parameters.subscriptionId,
    Parameters.resourceGroupName,
    Parameters.clusterName,
    Parameters.repairRunId,
    Parameters.intensityValue
  ],
  headerParameters: [Parameters.accept],
  serializer
};
